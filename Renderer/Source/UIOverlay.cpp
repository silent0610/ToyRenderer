...
void UIOverlay::PreparePipeline(const VkPipelineCache pipelineCache, const VkRenderPass renderPass, const VkFormat colorFormat, const VkFormat depthFormat)
{
	...
	VkPipelineMultisampleStateCreateInfo multisampleState =
		Init::pipelineMultisampleStateCreateInfo(rasterizationSamples);
	...
	VkPipelineDynamicStateCreateInfo dynamicState =
		Init::pipelineDynamicStateCreateInfo(dynamicStateEnables);
...
{
	// 定义一个 va_list 类型的变量 args，用于处理不定数量的参数
	va_list args;

	// 初始化 args，传入 formatstr，它是第一个参数
	va_start(args, formatstr);

	// 使用 ImGui 的 TextV 函数显示格式化的文本
	// TextV 用于处理变长参数，显示格式化后的字符串

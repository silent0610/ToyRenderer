...
namespace vkglTF
{
	enum DescriptorBindingFlags
	{
		...
		ImageMetallicRoughness = 0x00000003
	};
	...
	*/
	struct Texture
	{
		OldVulkanDevice *device = nullptr;
		VkImage image;
		...
		void destroy();
		void fromglTfImage(tinygltf::Image &gltfimage, std::string path, OldVulkanDevice *device, VkQueue copyQueue);
	};
	...
	*/
	struct Material
	{
		...
		Material(OldVulkanDevice *device) : device(device) {};
	...
	*/
	struct Mesh
	{
		...
		Mesh(OldVulkanDevice *device, glm::mat4 matrix);
		~Mesh();
	};
	...
	*/
	export enum class VertexComponent { Position,
										Normal,
										UV,
										Color,
										Tangent,
	...
	*/
	...
	public:
		...
		// Actual dimensions calculated from the final vertex buffer
		glm::vec3 actualDimensionsMin = glm::vec3(FLT_MAX);
		glm::vec3 actualDimensionsMax = glm::vec3(-FLT_MAX);
		...
		void loadImages(tinygltf::Model &gltfModel, OldVulkanDevice *device, VkQueue transferQueue);

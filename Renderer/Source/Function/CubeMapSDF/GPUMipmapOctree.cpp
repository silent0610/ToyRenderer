...
void GPUMipmapOctree::CreateUniformBuffer()
{
	...
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	allocInfo.allocationSize = memRequirements.size;
	allocInfo.memoryTypeIndex = m_device->GetMemoryType(memRequirements.memoryTypeBits,
...
{
	std::vector<OctreeNode> solidNodes;
...
OctreeNode GPUMipmapOctree::GetNode(glm::uvec3 position, uint32_t level)
{
	OctreeNode node;
	node.position = position;
	node.level = level;
	node.state = NodeState::EMPTY;
	node.size = CalculateNodeSize(level);
	node.center = CalculateNodeCenter(position, level);
...
std::vector<uint32_t> GPUMipmapOctree::ReadMipLevel(uint32_t level)
{
	...
	try
	{
		...
		vkCmdCopyImageToBuffer(commandBuffer, m_mipLevels[level].image,
...
void GPUMipmapOctree::PrintOctreeStatistics()
{
	std::cout << "=== GPU Mipmap Octree Statistics ===" << std::endl;
	std::cout << "Base size: " << m_baseSize << "³" << std::endl;
	std::cout << "Max levels: " << m_maxLevel << std::endl;

	uint32_t totalNodes = 0;
	for (uint32_t level = 0; level < m_maxLevel; level++)
	{
		uint32_t mipSize = CalculateMipSize(level);
		uint32_t levelNodes = mipSize * mipSize * mipSize;
		totalNodes += levelNodes;

		try
		{
			auto data = ReadMipLevel(level);
			uint32_t emptyCount = 0, mixedCount = 0, solidCount = 0;

			for (uint32_t value : data)
			{
				switch (value)
				{
				case static_cast<uint32_t>(NodeState::EMPTY):
					emptyCount++;
					break;
				case static_cast<uint32_t>(NodeState::MIXED):
					mixedCount++;
					break;
				case static_cast<uint32_t>(NodeState::SOLID):
					solidCount++;
					break;
				}
			}

			std::cout << "Level " << level << " (" << mipSize << "³): "
					  << "Empty=" << emptyCount << ", Mixed=" << mixedCount
					  << ", Solid=" << solidCount << std::endl;
		}
		catch (const std::exception &e)
		{
			std::cout << "Level " << level << " (" << mipSize << "³): Error reading - "
					  << e.what() << std::endl;
		}
	}
...
void GPUMipmapOctree::ValidateResults()
{
	std::cout << "=== Validating GPU Mipmap Octree Results ===" << std::endl;

	// Read all levels and validate parent-child relationships
	std::vector<std::vector<uint32_t>> allLevels(m_maxLevel);

	for (uint32_t level = 0; level < m_maxLevel; level++)
	{
		try
		{
			allLevels[level] = ReadMipLevel(level);
			std::cout << "✓ Successfully read level " << level << std::endl;
		}
		catch (const std::exception &e)
		{
			std::cout << "✗ Failed to read level " << level << ": " << e.what() << std::endl;
			return;
		}
	}
	...
	{
		uint32_t parentSize = CalculateMipSize(level);
		uint32_t childSize = CalculateMipSize(level - 1);

		for (uint32_t z = 0; z < parentSize; z++)
		{
			for (uint32_t y = 0; y < parentSize; y++)
			{
				for (uint32_t x = 0; x < parentSize; x++)
				{
					uint32_t parentIdx = z * parentSize * parentSize + y * parentSize + x;
					NodeState parentState = static_cast<NodeState>(allLevels[level][parentIdx]);

					// Check 8 children
					uint32_t solidChildren = 0, emptyChildren = 0;
					...
					for (uint32_t cz = 0; cz < 2; cz++)
					{
						for (uint32_t cy = 0; cy < 2; cy++)
						{
							...
							{
								uint32_t childX = x * 2 + cx;
								uint32_t childY = y * 2 + cy;
								uint32_t childZ = z * 2 + cz;

								if (childX < childSize && childY < childSize && childZ < childSize)
								{
									uint32_t childIdx = childZ * childSize * childSize + childY * childSize + childX;
									NodeState childState = static_cast<NodeState>(allLevels[level - 1][childIdx]);

									if (childState == NodeState::SOLID)
										solidChildren++;
									else if (childState == NodeState::EMPTY)
										emptyChildren++;
								}
							}
						}
					...
					bool valid = false;
					if (solidChildren == 8 && parentState == NodeState::SOLID)
						valid = true;
					else if (emptyChildren == 8 && parentState == NodeState::EMPTY)
						valid = true;
					else if (solidChildren < 8 && emptyChildren < 8 && parentState == NodeState::MIXED)
						valid = true;
					...
					if (!valid)
					{
						...
						{ // Only show first 5 errors
							std::cout << "✗ Validation error at level " << level
									  << " pos (" << x << "," << y << "," << z << "): "
									  << "Parent=" << static_cast<int>(parentState)
									  << ", SolidChildren=" << solidChildren
									  << ", EmptyChildren=" << emptyChildren << std::endl;
						}
					}
				}
			}
	...
	{
		std::cout << "✓ All parent-child relationships are valid!" << std::endl;
	}
	else
	{
		std::cout << "✗ Found " << validationErrors << " validation errors" << std::endl;
	}
...
void GPUMipmapOctree::CreateStagingBuffer(VkDeviceSize size, VkBuffer &buffer, VkDeviceMemory &memory)
{
	...
	allocInfo.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
	allocInfo.allocationSize = memRequirements.size;
	allocInfo.memoryTypeIndex = m_device->GetMemoryType(
...
std::vector<OctreeNode> GPUMipmapOctree::ExtractNodes(const NodeSelectionConfig &config)
{
	std::vector<OctreeNode> extractedNodes;

	for (uint32_t level = config.minLevel; level <= config.maxLevel && level < m_maxLevel; level++)
	{

		try
		{
			auto levelData = ReadMipLevel(level);
			uint32_t size = CalculateMipSize(level);
			uint32_t nodeCount = 0;

			for (uint32_t z = 0; z < size && nodeCount < config.maxNodesPerLevel; z++)
			{
				for (uint32_t y = 0; y < size && nodeCount < config.maxNodesPerLevel; y++)
				{
					for (uint32_t x = 0; x < size && nodeCount < config.maxNodesPerLevel; x++)
					{
						uint32_t index = z * size * size + y * size + x;
						NodeState nodeState = static_cast<NodeState>(levelData[index]);

						// Apply selection filters
						bool shouldInclude = false;
						if (nodeState == NodeState::SOLID)
						{
							shouldInclude = true;
						}
						else if (config.includeEmptyNodes && nodeState == NodeState::MIXED)
						{
							shouldInclude = true;
						}
						...
						if (shouldInclude)
						{
							...
							node.size = CalculateNodeSize(level);
							node.center = CalculateNodeCenter({x, y, z}, level);

							// Size filter
							if (node.size >= config.minNodeSize)
							{
								extractedNodes.push_back(node);
								nodeCount++;
							}
						}
					}
		...
		{
			std::cout << "Error reading level " << level << ": " << e.what() << std::endl;
		}
	}

	std::cout << "Total extracted nodes: " << extractedNodes.size() << std::endl;
	std::cout << "==============================" << std::endl;
...
void GPUMipmapOctree::ClearAllMipLevels()
{
	...
	for (uint32_t level = 0; level < m_maxLevel; level++)
	{
		...
		vkCmdClearColorImage(cmdBuffer, m_mipLevels[level].image,

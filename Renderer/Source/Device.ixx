...
/// @brief 封装和管理与 Vulkan 物理设备和逻辑设备相关的各种资源和操作。它使得在 Vulkan
/// 中处理设备、命令池、内存管理、队列等复杂的细节变得更加简单和清晰。具
export class OldVulkanDevice
{
public:
    ...
    explicit OldVulkanDevice(VkPhysicalDevice physicalDevice);
    ~OldVulkanDevice();
    ...
    VkResult CreateBuffer(VkBufferUsageFlags usageFlags, VkMemoryPropertyFlags memoryPropertyFlags, Buffer *buffer, VkDeviceSize size,
                          void *data = nullptr);

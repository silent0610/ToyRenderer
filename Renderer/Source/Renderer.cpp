...
Renderer::Renderer(Config *config) : config_(config)
{
    ...
    m_camera.setRotation(config->camera.Rotation);
    ...
    m_enableStableCameraSelection = config_->Sdf.EnableStableCameraSelection != 0;
    ...
    m_sdfIsoSurfacePass.pushConstants.volumeMax = glm::vec4(config_->Sdf.WorldSize * 0.5f, config_->Sdf.WorldSize * 0.5f,
                                                            config_->Sdf.WorldSize * 0.5f, 0.0f);
    m_sdfIsoSurfacePass.pushConstants.gridParams.x = static_cast<float>(config_->Sdf.SdfResolution);
}
...
void Renderer::InitVulkan()
{
    ...
    // GPU Timestamp性能统计 - 需要在device创建后立即初始化
...
void Renderer::ExportSDFDataForVisualization()
{
    ...
    {
        std::string outputFileName = GenerateSdfFileName("Analytical");
        std::string outputPath = Tool::GetAssetsPath() + "Sdf/" + outputFileName;
        ExportSDFDataForVisualization(GetAnalyticalSdfTexture(), VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, outputPath);
    }
...
void Renderer::ExportMeshToSdfData()
{
    std::string outputFileName = GenerateSdfFileName("JumpFlood");
    std::string outputPath = Tool::GetAssetsPath() + "Sdf/" + outputFileName;
...
void Renderer::SetUI(UIOverlay *overlay)
{
    ...
    if (overlay->Header("Settings"))
    {
        ...
        if (overlay->Header("AO"))
        {
            ...
            {
                int sampleCount = static_cast<int>(sdfAOPass_.buffers.cBufferData.sampleCount);
                if (overlay->InputInt("Sample Count", &sampleCount, 1))
                {
                    sdfAOPass_.buffers.cBufferData.sampleCount = static_cast<uint32_t>(sampleCount);
                    UpdateCBufferSdfAO();
                }
                if (overlay->InputFloat("Sample Radius", &sdfAOPass_.buffers.cBufferData.sampleRadius, 0.01f))
                {
                    UpdateCBufferSdfAO();
                }
                if (overlay->InputFloat("AO Strength", &sdfAOPass_.buffers.cBufferData.aoStrength, 0.01f))
                {
                    UpdateCBufferSdfAO();
                }
                if (overlay->InputFloat("Bias Distance", &sdfAOPass_.buffers.cBufferData.biasDistance, 0.01f))
                {
                    UpdateCBufferSdfAO();
                }
                if (overlay->InputFloat("Max Distance", &sdfAOPass_.buffers.cBufferData.maxDistance, 0.01f))
                {
                    UpdateCBufferSdfAO();
                }
                if (overlay->InputFloat("Falloff Power", &sdfAOPass_.buffers.cBufferData.falloffPower, 0.01f))
                {
                    UpdateCBufferSdfAO();
                }
            }
        }
        if (overlay->Header("Multiview"))
        {
            bool countSortEnabled = useCountSort_;
            if (overlay->CheckBox("Enable Count Sort", &countSortEnabled))
            {
            ...
            if (m_enableHierarchicalParentQuota)
            {
                ...
                bool stableSelectionEnabled = m_enableStableCameraSelection;
                if (overlay->CheckBox("Stable Camera Selection", &stableSelectionEnabled))
                {
                    SetStableCameraSelectionEnabled(stableSelectionEnabled);
                }
                if (m_enableStableCameraSelection)
                {
                    overlay->Text("stable: deterministic sort + temporal");
                }
                else
                {
                    overlay->Text("unstable: parallel atomics (faster)");
                }
                overlay->Text("Count Sort: forced off while hierarchical");
            }

            bool isoSurfaceEnabled = m_showMultiviewIsoSurface;
            if (overlay->CheckBox("Show Multiview IsoSurface", &isoSurfaceEnabled))
            {
                SetMultiviewIsoSurfaceEnabled(isoSurfaceEnabled);
                RecordMainCommandBuffer();
            }

            if (m_showMultiviewIsoSurface)
            {
                int axis = static_cast<int>(m_sdfIsoSurfacePass.pushConstants.sliceParams.x);
                if (overlay->ComboBox("Slice Axis", &axis, {"X", "Y", "Z"}))
                {
                    m_sdfIsoSurfacePass.pushConstants.sliceParams.x = static_cast<float>(glm::clamp(axis, 0, 2));
                    RecordMainCommandBuffer();
                }

                uint32_t maxLayer = 0;
                while ((1u << (maxLayer + 1)) <= config_->Sdf.SdfResolution)
                {
                    ++maxLayer;
                }

                int layer = static_cast<int>(m_sdfIsoSurfacePass.pushConstants.sliceParams.y);
                if (overlay->SliderInt("Octree Layer", &layer, 0, static_cast<int>(maxLayer)))
                {
                    layer = glm::clamp(layer, 0, static_cast<int>(maxLayer));
                    m_sdfIsoSurfacePass.pushConstants.sliceParams.y = static_cast<float>(layer);
                    uint32_t sliceCount = std::max(1u, config_->Sdf.SdfResolution >> layer);
                    int sliceIndex = static_cast<int>(m_sdfIsoSurfacePass.pushConstants.sliceParams.z);
                    sliceIndex = glm::clamp(sliceIndex, 0, static_cast<int>(sliceCount) - 1);
                    m_sdfIsoSurfacePass.pushConstants.sliceParams.z = static_cast<float>(sliceIndex);
                    RecordMainCommandBuffer();
                }

                uint32_t sliceCount = std::max(1u, config_->Sdf.SdfResolution >> static_cast<uint32_t>(m_sdfIsoSurfacePass.pushConstants.sliceParams.y));
                int sliceIndex = static_cast<int>(m_sdfIsoSurfacePass.pushConstants.sliceParams.z);
                if (overlay->SliderInt("Slice Index", &sliceIndex, 0, static_cast<int>(sliceCount) - 1))
                {
                    sliceIndex = glm::clamp(sliceIndex, 0, static_cast<int>(sliceCount) - 1);
                    m_sdfIsoSurfacePass.pushConstants.sliceParams.z = static_cast<float>(sliceIndex);
                    RecordMainCommandBuffer();
                ...
                float contourWidth = m_sdfIsoSurfacePass.pushConstants.sliceParams.w;
                if (overlay->SliderFloat("Contour Width", &contourWidth, 0.001f, 0.05f))
                {
                    m_sdfIsoSurfacePass.pushConstants.sliceParams.w = contourWidth;
                    RecordMainCommandBuffer();
                }
            }
        }
...
void Renderer::Draw()
{
...

    {
        ...
        // 重置QueryPool（每帧开始时）
        vkCmdResetQueryPool(meshToSdfCommandBuffer_, timestampQueryPool_, 0, MAX_TIMESTAMP_QUERIES);

        // 开始MeshToSdf时间测量
        vkCmdWriteTimestamp(meshToSdfCommandBuffer_, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestampQueryPool_, MESHTOSDF_START);
        ...
        // 结束MeshToSdf时间测量
        vkCmdWriteTimestamp(meshToSdfCommandBuffer_, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestampQueryPool_, MESHTOSDF_END);
...
void Renderer::InitVoxelizationPipelines()
{
    ...
    // conservativeRasterInfo.extraPrimitiveOverestimationSize = 0.0f; // 可以设置为0，或者根据需要调整
...
void Renderer::Cleanup()
{
    ...
    if (m_pipelines.sdfIsoSurface != VK_NULL_HANDLE)
    {
        vkDestroyPipeline(m_device, m_pipelines.sdfIsoSurface, nullptr);
        m_pipelines.sdfIsoSurface = VK_NULL_HANDLE;
    }
    if (m_pipelineLayouts.sdfIsoSurface != nullptr)
    {
        vkDestroyPipelineLayout(m_device, m_pipelineLayouts.sdfIsoSurface, nullptr);
        m_pipelineLayouts.sdfIsoSurface = nullptr;
    }
    if (m_descriptorSetLayouts.sdfIsoSurface != nullptr)
    {
        vkDestroyDescriptorSetLayout(m_device, m_descriptorSetLayouts.sdfIsoSurface, nullptr);
        m_descriptorSetLayouts.sdfIsoSurface = nullptr;
    }
...
void Renderer::RecordMainCommandBuffer()
{
    ...
    // Camera overlay pass
    if (m_showMultiviewIsoSurface || m_enableCameraOverlay)
    {
        ...
        if (m_showMultiviewIsoSurface)
        {
            vkCmdBindPipeline(m_offScreenCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelines.sdfIsoSurface);
            vkCmdBindDescriptorSets(m_offScreenCmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, m_pipelineLayouts.sdfIsoSurface, 0, 1,
                                    &m_descriptorSets.sdfIsoSurface, 0, NULL);
            vkCmdPushConstants(m_offScreenCmdBuffer, m_pipelineLayouts.sdfIsoSurface,
                               VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0,
                               sizeof(SdfIsoSurfacePass::PushConstants), &m_sdfIsoSurfacePass.pushConstants);
            vkCmdDraw(m_offScreenCmdBuffer, 6, 1, 0, 0);
        }
        else
        {
...
void Renderer::SetupDescriptors()
{
    ...
    setLayoutBindings = {
        Init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 0),
        Init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, 1),
    };
    descriptorLayoutCI = Init::descriptorSetLayoutCreateInfo(setLayoutBindings);
    Tool::CheckResult(vkCreateDescriptorSetLayout(m_device, &descriptorLayoutCI, nullptr, &m_descriptorSetLayouts.sdfIsoSurface));
...
void Renderer::AllocateDescriptorSetSdfIsoSurface()
{
    if (m_descriptorSetLayouts.sdfIsoSurface == nullptr)
    {
        return;
    }
    if (m_multiViewDepthSDF4C.sdfFusionPass.finalSDFTexture == VK_NULL_HANDLE ||
        m_multiViewDepthSDF4C.sdfFusionPass.finalSDFSampler == VK_NULL_HANDLE)
    {
        return;
    }

    if (m_descriptorSets.sdfIsoSurface == nullptr)
    {
        VkDescriptorSetAllocateInfo allocInfo = Init::descriptorSetAllocateInfo(m_descriptorPool, &m_descriptorSetLayouts.sdfIsoSurface, 1);
        Tool::CheckResult(vkAllocateDescriptorSets(m_device, &allocInfo, &m_descriptorSets.sdfIsoSurface));
    }

    VkDescriptorImageInfo sdfDescriptor = m_multiViewDepthSDF4C.sdfFusionPass.finalSDFDescriptor;
    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        Init::writeDescriptorSet(m_descriptorSets.sdfIsoSurface, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &m_sharedBuffers.ConstBufferCamera.descriptor),
        Init::writeDescriptorSet(m_descriptorSets.sdfIsoSurface, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &sdfDescriptor),
    };
    vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}
...
void Renderer::UpdateFXAADescriptorSet()
{
    if (m_descriptorSets.FXAA == nullptr || m_framebuffers.ToneMapping == nullptr)
    {
        return;
    }

    const bool useCameraOverlay = m_enableCameraOverlay || m_showMultiviewIsoSurface;
    if (useCameraOverlay && m_framebuffers.CameraOverlay == nullptr)
    {
        return;
    }

    VkDescriptorImageInfo sourceColor = Init::descriptorImageInfo(
        useCameraOverlay ? m_framebuffers.CameraOverlay->sampler : m_framebuffers.ToneMapping->sampler,
        useCameraOverlay ? m_framebuffers.CameraOverlay->attachments[0].view : m_framebuffers.ToneMapping->attachments[0].view,
        VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);

    std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
        Init::writeDescriptorSet(m_descriptorSets.FXAA, VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &m_uniformBuffers.FXAA.descriptor),
        Init::writeDescriptorSet(m_descriptorSets.FXAA, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 1, &sourceColor),
    };
    vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(writeDescriptorSets.size()), writeDescriptorSets.data(), 0, nullptr);
}
void Renderer::PreparePipelineSdfIsoSurface()
{
    if (m_pipelineLayouts.sdfIsoSurface != nullptr)
    {
        vkDestroyPipelineLayout(m_device, m_pipelineLayouts.sdfIsoSurface, nullptr);
    ...
    if (m_pipelines.sdfIsoSurface != nullptr)
    {
        vkDestroyPipeline(m_device, m_pipelines.sdfIsoSurface, nullptr);
        m_pipelines.sdfIsoSurface = nullptr;
    }

    VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo = Init::pipelineLayoutCreateInfo(&m_descriptorSetLayouts.sdfIsoSurface, 1);
    VkPushConstantRange pushConstantRange =
        Init::pushConstantRange(VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, sizeof(SdfIsoSurfacePass::PushConstants), 0);
    pipelineLayoutCreateInfo.pushConstantRangeCount = 1;
    pipelineLayoutCreateInfo.pPushConstantRanges = &pushConstantRange;
    Tool::CheckResult(vkCreatePipelineLayout(m_device, &pipelineLayoutCreateInfo, nullptr, &m_pipelineLayouts.sdfIsoSurface));
    ...
    VkGraphicsPipelineCreateInfo pipelineCI = Init::pipelineCreateInfo(m_pipelineLayouts.sdfIsoSurface, m_framebuffers.CameraOverlay->renderPass);
    ...
    shaderStages[0] = LoadShader(Tool::GetShadersPath() + "Post/SdfIsoSurface.Vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    shaderStages[1] = LoadShader(Tool::GetShadersPath() + "Post/SdfIsoSurface.Frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    Tool::CheckResult(vkCreateGraphicsPipelines(m_device, m_pipelineCache, 1, &pipelineCI, nullptr, &m_pipelines.sdfIsoSurface));
}
...
void Renderer::InitializeAnalyticalNodeSelectionResource()
{
    ...
    Tool::CheckResult(m_vulkanDevice->CreateBuffer(
        ...
        &analyticalNodeSelection_.solidNodeBuffer, sizeof(AnalyticalSolidNodeSelection::SolidNode) * config_->Sdf.AnalyticalUsedPointNum));
    ...
    solidNodeBufferInfo.range = analyticalNodeSelection_.solidNodeBuffer.size;
...
void Renderer::InitializeMultiviewNodeSelectionResource()
{
    ...
    Tool::CheckResult(m_vulkanDevice->CreateBuffer(
        VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, &multiViewNodeSelection_.prevSelectedNodesBuffer,
        sizeof(MultiViewSolidNodeSelection::SolidNode) * config_->Sdf.MultiViewUsedCameraNum));
    Tool::CheckResult(m_vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
                                                   VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
                                                   &multiViewNodeSelection_.prevSelectedCountBuffer, sizeof(uint32_t)));
    m_invalidatePrevCameraSelection = true;
    Tool::CheckResult(m_vulkanDevice->CreateBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
                                                   VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
    ...
    std::string collectionShaderPath{};
    // 创建收集管线
    if (config_->Sdf.UseRandomSelection)
    {
        collectionShaderPath = Tool::GetShadersPath() + "MultiView/MultiViewNodeSelectionRandom.Comp.spv";
    }
    else
    {
        collectionShaderPath = Tool::GetShadersPath() + "MultiView/MultiViewNodeSelection.Comp.spv";
    }
    ...
    collectionDescriptorWrites.push_back(levelCountWrite);
    ...
    VkDescriptorSetLayoutBinding levelCountBinding{};
    levelCountBinding.binding = 4;
    levelCountBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    levelCountBinding.descriptorCount = 1;
    ...
    VkDescriptorSetLayoutBinding prevSelectedCountBinding{};
    prevSelectedCountBinding.binding = 7;
    prevSelectedCountBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    prevSelectedCountBinding.descriptorCount = 1;
    prevSelectedCountBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    finalSelectionBindings.push_back(prevSelectedCountBinding);

    VkDescriptorSetLayoutBinding prevSelectedNodesBinding{};
    prevSelectedNodesBinding.binding = 8;
    prevSelectedNodesBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    prevSelectedNodesBinding.descriptorCount = 1;
    prevSelectedNodesBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
    finalSelectionBindings.push_back(prevSelectedNodesBinding);
    ...
    VkWriteDescriptorSet prevSelectedCountWrite{};
    prevSelectedCountWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    prevSelectedCountWrite.dstSet = multiViewNodeSelection_.finalSelectionDescriptorSet;
    prevSelectedCountWrite.dstBinding = 7;
    prevSelectedCountWrite.dstArrayElement = 0;
    prevSelectedCountWrite.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    prevSelectedCountWrite.descriptorCount = 1;
    prevSelectedCountWrite.pBufferInfo = &multiViewNodeSelection_.prevSelectedCountBuffer.descriptor;
    finalSelectionDescriptorWrites.push_back(prevSelectedCountWrite);

    VkWriteDescriptorSet prevSelectedNodesWrite{};
    prevSelectedNodesWrite.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
    prevSelectedNodesWrite.dstSet = multiViewNodeSelection_.finalSelectionDescriptorSet;
    prevSelectedNodesWrite.dstBinding = 8;
    prevSelectedNodesWrite.dstArrayElement = 0;
    prevSelectedNodesWrite.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    prevSelectedNodesWrite.descriptorCount = 1;
    prevSelectedNodesWrite.pBufferInfo = &multiViewNodeSelection_.prevSelectedNodesBuffer.descriptor;
    finalSelectionDescriptorWrites.push_back(prevSelectedNodesWrite);
    ...
    vkDestroyShaderModule(m_device, countSortShader, nullptr);

    std::string passthroughShaderPath = Tool::GetShadersPath() + "MultiView/CountSortPassthrough.Comp.spv";
    VkShaderModule passthroughShader = Tool::LoadShader(passthroughShaderPath.c_str(), m_device);
    VkPipelineShaderStageCreateInfo passthroughShaderStageInfo{};
    passthroughShaderStageInfo.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
    passthroughShaderStageInfo.stage = VK_SHADER_STAGE_COMPUTE_BIT;
    passthroughShaderStageInfo.module = passthroughShader;
    passthroughShaderStageInfo.pName = "main";
    VkComputePipelineCreateInfo passthroughPipelineInfo{};
    passthroughPipelineInfo.sType = VK_STRUCTURE_TYPE_COMPUTE_PIPELINE_CREATE_INFO;
    passthroughPipelineInfo.layout = multiViewNodeSelection_.SortingPipelineLayout;
    passthroughPipelineInfo.stage = passthroughShaderStageInfo;
    Tool::CheckResult(vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &passthroughPipelineInfo, nullptr,
                                               &multiViewNodeSelection_.SortingPassthroughPipeline));
    vkDestroyShaderModule(m_device, passthroughShader, nullptr);
...
void Renderer::ExecuteMultiViewNodeSelection(VkCommandBuffer commandBuffer)
{
    ...
    std::vector<VkBufferMemoryBarrier> fillBarriers = {
    ...
    if (m_invalidatePrevCameraSelection)
    {
        vkCmdFillBuffer(commandBuffer, multiViewNodeSelection_.prevSelectedCountBuffer.buffer, 0, sizeof(uint32_t), 0);
        fillBarriers.push_back(
            bufferBarrier(multiViewNodeSelection_.prevSelectedCountBuffer.buffer, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT));
        m_invalidatePrevCameraSelection = false;
    }
    vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr,
    ...
    multiViewNodeSelection_.FinalSelectionPushConstant.UseStableSelection = m_enableStableCameraSelection ? 1u : 0u;

    const bool hierarchical = m_enableHierarchicalParentQuota;
    const bool stableHierarchical = hierarchical && m_enableStableCameraSelection;
    ...
    auto dispatchFinalSelection = [&](int level, uint32_t estimatedCandidates) {
        ...
        // 稳定路径：1 个 256 线程 workgroup（bitonic）；不稳定/旧路径：按候选数并行
        if (stableHierarchical)
        {
            vkCmdDispatch(commandBuffer, 1, 1, 1);
        }
        else
        {
            const uint32_t groups = std::max(1u, (estimatedCandidates + 255u) / 256u);
    ...
    if (hierarchical)
    {
        ...
        for (int level = coarsestLevel; level >= finestLevel; --level)
        {
            ...
            VkBufferMemoryBarrier afterCollect[] = {
                bufferBarrier(multiViewNodeSelection_.candidateCountBuffer.buffer, VK_ACCESS_SHADER_WRITE_BIT,
                              VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
                // Final 层级路径会原地排序候选，需要读写
                bufferBarrier(multiViewNodeSelection_.candidateNodesBuffer.buffer, VK_ACCESS_SHADER_WRITE_BIT,
                              VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
            };
        ...
        // 拷贝本帧选中结果 → prev，供下一帧时序优先（仅稳定路径）
        if (stableHierarchical)
        {
            VkBufferMemoryBarrier beforeCopy[] = {
                bufferBarrier(multiViewNodeSelection_.selectedCountBuffer.buffer, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT),
                bufferBarrier(multiViewNodeSelection_.selectedNodesBuffer.buffer, VK_ACCESS_SHADER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT),
            };
            vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 2, beforeCopy, 0,
                                 nullptr);

            VkBufferCopy countCopy{0, 0, sizeof(uint32_t)};
            vkCmdCopyBuffer(commandBuffer, multiViewNodeSelection_.selectedCountBuffer.buffer,
                            multiViewNodeSelection_.prevSelectedCountBuffer.buffer, 1, &countCopy);

            const VkDeviceSize nodeBytes =
                sizeof(MultiViewSolidNodeSelection::SolidNode) * static_cast<VkDeviceSize>(config_->Sdf.MultiViewUsedCameraNum);
            VkBufferCopy nodesCopy{0, 0, nodeBytes};
            vkCmdCopyBuffer(commandBuffer, multiViewNodeSelection_.selectedNodesBuffer.buffer,
                            multiViewNodeSelection_.prevSelectedNodesBuffer.buffer, 1, &nodesCopy);

            VkBufferMemoryBarrier afterCopy[] = {
                bufferBarrier(multiViewNodeSelection_.prevSelectedCountBuffer.buffer, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT),
                bufferBarrier(multiViewNodeSelection_.prevSelectedNodesBuffer.buffer, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_SHADER_READ_BIT),
                bufferBarrier(multiViewNodeSelection_.selectedCountBuffer.buffer, VK_ACCESS_TRANSFER_READ_BIT,
                              VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
                bufferBarrier(multiViewNodeSelection_.selectedNodesBuffer.buffer, VK_ACCESS_TRANSFER_READ_BIT,
                              VK_ACCESS_SHADER_READ_BIT | VK_ACCESS_SHADER_WRITE_BIT),
            };
            vkCmdPipelineBarrier(commandBuffer, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_COMPUTE_SHADER_BIT, 0, 0, nullptr, 4, afterCopy, 0,
                                 nullptr);
        }
    }
    else
...
void Renderer::MultiViewSolidNodeSelection::cleanup(VkDevice device)
{
    ...
    if (SortingPipeline != VK_NULL_HANDLE)
    {
        vkDestroyPipeline(device, SortingPipeline, nullptr);
        SortingPipeline = VK_NULL_HANDLE;
    }
    if (SortingPassthroughPipeline != VK_NULL_HANDLE)
    {
        vkDestroyPipeline(device, SortingPassthroughPipeline, nullptr);
        SortingPassthroughPipeline = VK_NULL_HANDLE;
    }
    if (SortingPipelineLayout != VK_NULL_HANDLE)
    {
        vkDestroyPipelineLayout(device, SortingPipelineLayout, nullptr);
        SortingPipelineLayout = VK_NULL_HANDLE;
    }
    if (SortingDescriptorSetLayout != VK_NULL_HANDLE)
    {
        vkDestroyDescriptorSetLayout(device, SortingDescriptorSetLayout, nullptr);
        SortingDescriptorSetLayout = VK_NULL_HANDLE;
    }
    ...
    prevSelectedNodesBuffer.Destroy();
    prevSelectedCountBuffer.Destroy();
...
void Renderer::SetMultiViewCountSortEnabled(bool enabled)
{
    if (useCountSort_ != enabled)
    {
        useCountSort_ = enabled;
        printf("Switched MultiView CountSort %s\n", enabled ? "Enabled" : "Disabled");

        if (m_unifiedGPUPipeline.commandsRecorded)
        {
            m_unifiedGPUPipeline.commandsRecorded = false;
            printf("Pipeline commands will be re-recorded with new count-sort mode\n");
        }
    }
}

void Renderer::SetMultiviewIsoSurfaceEnabled(bool enabled)
{
    if (m_showMultiviewIsoSurface != enabled)
    {
        m_showMultiviewIsoSurface = enabled;
        printf("Switched Multiview IsoSurface %s\n", enabled ? "Enabled" : "Disabled");
        UpdateFXAADescriptorSet();
        if (m_unifiedGPUPipeline.commandsRecorded)
        {
            m_unifiedGPUPipeline.commandsRecorded = false;
            printf("Pipeline commands will be re-recorded with new iso-surface mode\n");
        }
    }
}

void Renderer::SetCameraOverlayEnabled(bool enabled)
{
    if (m_enableCameraOverlay != enabled)
    {
        m_enableCameraOverlay = enabled;
        printf("Switched Camera Overlay %s\n", enabled ? "Enabled" : "Disabled");
        UpdateFXAADescriptorSet();
        if (m_unifiedGPUPipeline.commandsRecorded)
        {
            m_unifiedGPUPipeline.commandsRecorded = false;
            printf("Pipeline commands will be re-recorded with new camera-overlay mode\n");
        }
    }
}
...
void Renderer::SetStableCameraSelectionEnabled(bool enabled)
{
    if (m_enableStableCameraSelection == enabled)
    {
        return;
    }
    m_enableStableCameraSelection = enabled;
    config_->Sdf.EnableStableCameraSelection = enabled ? 1u : 0u;
    m_invalidatePrevCameraSelection = true;
    printf("Stable Camera Selection %s\n", enabled ? "Enabled" : "Disabled");
    if (m_unifiedGPUPipeline.commandsRecorded)
    {
        m_unifiedGPUPipeline.commandsRecorded = false;
        printf("Pipeline commands will be re-recorded with stable selection mode\n");
    }
}
...
void Renderer::ExecuteAnalyticalNodeSelection(VkCommandBuffer cmd)
{
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_COMPUTE, analyticalNodeSelection_.pipelineLayout, 0, 1,
...
void Renderer::RecordUnifiedGPUPipelineCommands()
{
    ...
    // 开始UnifiedPipeline时间测量
    vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, timestampQueryPool_, UNIFIED_PIPELINE_START);
    ...
    if (useMultiview_)
    {
        BeginDebugLabel(cmd, "Multiview Node Selection ", 0.0f, 1.0f, 0.5f, 1.0f);
        ExecuteMultiViewNodeSelection(cmd);
        EndDebugLabel(cmd);
    }
    else
    {
        BeginDebugLabel(cmd, "Analytical Node Selection", 0.0f, 1.0f, 1.0f, 1.0f);
    ...
    vkCmdWriteTimestamp(cmd, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, timestampQueryPool_, UNIFIED_PIPELINE_END);
...
void Renderer::CreateCameraMatrixPreparationPipeline()
{
    ...
    VkPipelineShaderStageCreateInfo shaderStageInfo =
        LoadShader(Tool::GetShadersPath() + "Multiview/CameraMatrixPreparation.Comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
...
void Renderer::CreateIndirectCommandGenerationPipeline()
{
    ...
    // 3. 加载着色器并创建计算管线
    VkPipelineShaderStageCreateInfo shaderStageInfo =
        LoadShader(Tool::GetShadersPath() + "Multiview/IndirectCommandGeneration.Comp.spv", VK_SHADER_STAGE_COMPUTE_BIT);
...
void Renderer::CreateDepthCubemapArray()
{
    ...
    imageInfo.imageType = VK_IMAGE_TYPE_2D;
    imageInfo.format = VK_FORMAT_R32_SFLOAT;
    imageInfo.extent.width = config_->Sdf.MultiViewDepthResolution;  // DepthRenderingPass4C::CUBEMAP_SIZE
    imageInfo.extent.height = config_->Sdf.MultiViewDepthResolution; // DepthRenderingPass4C::CUBEMAP_SIZE
    imageInfo.extent.depth = 1;
    imageInfo.mipLevels = 1;
...
void Renderer::CreateMultiViewDepthRenderPass()
{
    ...
    depthImageInfo.extent.width = config_->Sdf.MultiViewDepthResolution;  // DepthRenderingPass4C::CUBEMAP_SIZE
    depthImageInfo.extent.height = config_->Sdf.MultiViewDepthResolution; // DepthRenderingPass4C::CUBEMAP_SIZE
    ...
    framebufferInfo.width = config_->Sdf.MultiViewDepthResolution;
    framebufferInfo.height = config_->Sdf.MultiViewDepthResolution;
...
void Renderer::CreateMultiViewDepthPipeline()
{
    ...
    // 3. 加载着色器
    VkPipelineShaderStageCreateInfo vertShader = LoadShader(Tool::GetShadersPath() + "MultiView/MultiViewDepth.Vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
    VkPipelineShaderStageCreateInfo fragShader =
        LoadShader(Tool::GetShadersPath() + "MultiView/MultiViewDepth.Frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    viewport.x = 0.0f;
    viewport.y = 0.0f;
    viewport.width = static_cast<float>(config_->Sdf.MultiViewDepthResolution);  // static_cast<float>(DepthRenderingPass4C::CUBEMAP_SIZE)
    viewport.height = static_cast<float>(config_->Sdf.MultiViewDepthResolution); // static_cast<float>(DepthRenderingPass4C::CUBEMAP_SIZE)
    viewport.minDepth = 0.0f;
    viewport.maxDepth = 1.0f;

    VkRect2D scissor{};
    scissor.offset = {0, 0};
    scissor.extent = {config_->Sdf.MultiViewDepthResolution,
                      config_->Sdf.MultiViewDepthResolution}; // {DepthRenderingPass4C::CUBEMAP_SIZE, DepthRenderingPass4C::CUBEMAP_SIZE}
    ...
    writes.push_back(Init::writeDescriptorSet(depthPass.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 1,
                                              &m_multiViewDepthSDF4C.gpuPreparation.activeCameraCountBuffer.descriptor));
...
void Renderer::ExecuteMultiViewDepthRendering(VkCommandBuffer cmd)
{
    ...
    renderPassInfo.renderArea.extent = {config_->Sdf.MultiViewDepthResolution, config_->Sdf.MultiViewDepthResolution};
...
{
    if (m_multiViewDepthSDF4C.sdfFusionPass.finalSDFSampler != VK_NULL_HANDLE)
    {
        return;
    }
...
void Renderer::CreateSDFFusionPipeline()
{
    ...
    Tool::CheckResult(vkCreateDescriptorSetLayout(m_device, &layoutInfo, nullptr, &m_multiViewDepthSDF4C.sdfFusionPass.descriptorSetLayout));
    ...
    Tool::CheckResult(vkCreatePipelineLayout(m_device, &pipelineLayoutInfo, nullptr, &m_multiViewDepthSDF4C.sdfFusionPass.pipelineLayout));

    // Step 4: Load compute shader
    std::string shaderPath = Tool::GetShadersPath() + "Multiview/SDFFusion.Comp.spv";
    ...
    Tool::CheckResult(
        vkCreateComputePipelines(m_device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &m_multiViewDepthSDF4C.sdfFusionPass.computePipeline));
...
void Renderer::InitializeSDFFusionPass()
{
    ...
    Tool::CheckResult(vkAllocateDescriptorSets(m_device, &allocInfo, &m_multiViewDepthSDF4C.sdfFusionPass.descriptorSet));
    ...
    descriptorWrites[4].dstSet = m_multiViewDepthSDF4C.sdfFusionPass.descriptorSet;
    descriptorWrites[4].dstBinding = 4;
    descriptorWrites[4].dstArrayElement = 0;
    descriptorWrites[4].descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
    descriptorWrites[4].descriptorCount = 1;
    descriptorWrites[4].pBufferInfo = &m_multiViewDepthSDF4C.gpuPreparation.activeCameraCountBuffer.descriptor;
    vkUpdateDescriptorSets(m_device, static_cast<uint32_t>(descriptorWrites.size()), descriptorWrites.data(), 0, nullptr);
...
void Renderer::UpdateCBufferSdfAO()
{
    ...
    // sdfAOPass_.buffers.cBufferData.sampleRadius = 0.10f;
    // sdfAOPass_.buffers.cBufferData.aoStrength = 0.30f;
...
void Renderer::AllocateDescriptorSetSdfAO()
{
    ...
    AllocateDescriptorSetSdfIsoSurface();
}
...
void Renderer::ExportAOData()
{
    ...
    Tool::SetImageLayout(cmdBuffer, aoImage, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, range);
    ...
    // 2.3 执行图像到buffer的拷贝
    vkCmdCopyImageToBuffer(cmdBuffer, aoImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, stagingBuffer.buffer, 1, &region);

    // 2.4 恢复图像布局: TRANSFER_SRC -> SHADER_READ_ONLY
    Tool::SetImageLayout(cmdBuffer, aoImage, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, range);
    ...
    // 6. 保存为PNG文件
    std::string savePath = Tool::GetAssetsPath() + "Sdf/" + modelName + ".png";
...
void Renderer::InitializeTimestampQueries()
{
    // 获取GPU时间戳周期
    VkPhysicalDeviceProperties deviceProps{};
    vkGetPhysicalDeviceProperties(m_physicalDevice, &deviceProps);
    timestampPeriod_ = deviceProps.limits.timestampPeriod;
    ...
    VkResult result = vkCreateQueryPool(m_device, &queryPoolInfo, nullptr, &timestampQueryPool_);
    if (result != VK_SUCCESS)
    {
    ...
    // 预分配vector容量
    perfStats_.meshToSdfTimes.reserve(perfStats_.targetFrames);
    perfStats_.unifiedPipelineTimes.reserve(perfStats_.targetFrames);

    printf("GPU Timestamp initialized. Warmup: %u frames, Target: %u frames\n", perfStats_.warmupFrames, perfStats_.targetFrames);
}

void Renderer::CleanupTimestampQueries()
{
    if (timestampQueryPool_ != VK_NULL_HANDLE)
    {
        vkDestroyQueryPool(m_device, timestampQueryPool_, nullptr);
...
void Renderer::CollectFrameTimestamps()
{
    ...
    if (perfStats_.currentFrame <= perfStats_.warmupFrames)
    {
        if (perfStats_.currentFrame == perfStats_.warmupFrames)
        {
            printf("\n=== Warmup complete, starting data collection ===\n");
...
void Renderer::PrintPerformanceStatistics()
{
    if (perfStats_.meshToSdfTimes.empty() || perfStats_.unifiedPipelineTimes.empty())
    {
    ...
    auto calculateStats = [](const std::vector<float> &times)
    {
        ...
        stats.p95 = sorted[size_t(n * 0.95)];
        stats.p99 = sorted[size_t(n * 0.99)];

        return stats;
    };

    auto meshStats = calculateStats(perfStats_.meshToSdfTimes);
    auto pipelineStats = calculateStats(perfStats_.unifiedPipelineTimes);

    printf("\n");
    printf("========================================\n");
    printf("  GPU Performance Statistics\n");
    printf("  (Collected over %zu frames)\n", perfStats_.meshToSdfTimes.size());
    printf("========================================\n\n");

    printf("MeshToSdf GPU Time:\n");
    printf("  Mean:      %.3f ± %.3f ms\n", meshStats.mean, meshStats.stddev);
    printf("  Median:    %.3f ms\n", meshStats.median);
    printf("  Min/Max:   %.3f / %.3f ms\n", meshStats.min, meshStats.max);
    printf("  P95/P99:   %.3f / %.3f ms\n", meshStats.p95, meshStats.p99);
    printf("\n");

    printf("UnifiedPipeline GPU Time:\n");
    printf("  Mean:      %.3f ± %.3f ms\n", pipelineStats.mean, pipelineStats.stddev);
    printf("  Median:    %.3f ms\n", pipelineStats.median);
    printf("  Min/Max:   %.3f / %.3f ms\n", pipelineStats.min, pipelineStats.max);
    printf("  P95/P99:   %.3f / %.3f ms\n", pipelineStats.p95, pipelineStats.p99);
    printf("\n");

    printf("Total GPU Time per Frame:\n");
    printf("  Mean:      %.3f ms\n", meshStats.mean + pipelineStats.mean);
    printf("  Est. FPS:  %.1f (GPU-bound)\n", 1000.0f / (meshStats.mean + pipelineStats.mean));
    printf("\n");
    printf("========================================\n");
    ...
    if (f)
    {
        ...
        fprintf(f, "Frames: %zu\n\n", perfStats_.meshToSdfTimes.size());
        fprintf(f, "MeshToSdf: %.3f ± %.3f ms (median: %.3f, P95: %.3f, P99: %.3f)\n", meshStats.mean, meshStats.stddev, meshStats.median,
                meshStats.p95, meshStats.p99);
        fprintf(f, "UnifiedPipeline: %.3f ± %.3f ms (median: %.3f, P95: %.3f, P99: %.3f)\n", pipelineStats.mean, pipelineStats.stddev,
                pipelineStats.median, pipelineStats.p95, pipelineStats.p99);
        fprintf(f, "Total: %.3f ms\n", meshStats.mean + pipelineStats.mean);
        fclose(f);
        printf("Performance log saved to: %s\n", logPath.c_str());
    }
}

...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
	...
	Tool::SetImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, subresourceRange);
...
void vkglTF::Model::loadFromFile(std::string filename, OldVulkanDevice *device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
	...
	if ((fileLoadingFlags & FileLoadingFlags::PreTransformVertices) || (fileLoadingFlags & FileLoadingFlags::PreMultiplyVertexColors) || (fileLoadingFlags & FileLoadingFlags::FlipY))
	{
		...
		for (Node *node : linearNodes)
		{
			if (node->mesh)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					std::cout << "  " << localMatrix[0][i] << "\t" << localMatrix[1][i] << "\t" << localMatrix[2][i] << "\t" << localMatrix[3][i] << std::endl;
				}
	...
	// Manually record the actual min and max from vertexBuffer for verification
	{
		...
		std::cout << "Calculated Min (from getSceneDimensions): (" << dimensions.min.x << ", " << dimensions.min.y << ", " << dimensions.min.z << ")" << std::endl;
		std::cout << "Calculated Max (from getSceneDimensions): (" << dimensions.max.x << ", " << dimensions.max.y << ", " << dimensions.max.z << ")" << std::endl;
		std::cout << "-------------------------------------\n"
				  << std::endl;
	}

...
*/
bool loadImageDataFunc(tinygltf::Image *image, const int imageIndex, std::string *error, std::string *warning, int req_width, int req_height,
...
void vkglTF::Texture::fromglTfImage(tinygltf::Image &gltfimage, std::string path, OldVulkanDevice *device, VkQueue copyQueue)
{
    this->device = device;
    ...
    if (!isKtx)
    {
        ...
        vkCmdPipelineBarrier(copyCmd, VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1,
                             &imageMemoryBarrier);
        ...
        imageMemoryBarrier.image = image;
        imageMemoryBarrier.subresourceRange = subresourceRange;
        vkCmdPipelineBarrier(copyCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, 0, 0, nullptr, 0, nullptr, 1,
                             &imageMemoryBarrier);
        ...
        imageMemoryBarrier.image = image;
        imageMemoryBarrier.subresourceRange = subresourceRange;
        vkCmdPipelineBarrier(blitCmd, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0, nullptr, 1,
                             &imageMemoryBarrier);
                                           ...
                                           uint32_t descriptorBindingFlags)
{
    ...
    Tool::CheckResult(vkAllocateDescriptorSets(device->logicalDevice, &descriptorSetAllocInfo, &descriptorSet));
...
/*
    glTF mesh
*/
vkglTF::Mesh::Mesh(OldVulkanDevice *device, glm::mat4 matrix)
{
    this->device = device;
    this->uniformBlock.matrix = matrix;
    Tool::CheckResult(device->CreateBuffer(VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT,
    ...
    Tool::CheckResult(vkMapMemory(device->logicalDevice, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped));
...
void vkglTF::Model::createEmptyTexture(VkQueue transferQueue)
{
    ...
    imageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
    Tool::CheckResult(vkCreateImage(device->logicalDevice, &imageCreateInfo, nullptr, &emptyTexture.image));

    vkGetImageMemoryRequirements(device->logicalDevice, emptyTexture.image, &memReqs);
    memAllocInfo.allocationSize = memReqs.size;
    memAllocInfo.memoryTypeIndex = device->GetMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
    Tool::CheckResult(vkAllocateMemory(device->logicalDevice, &memAllocInfo, nullptr, &emptyTexture.deviceMemory));
    Tool::CheckResult(vkBindImageMemory(device->logicalDevice, emptyTexture.image, emptyTexture.deviceMemory, 0));
    ...
    VkCommandBuffer copyCmd = device->CreateCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
    Tool::SetImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, subresourceRange);
    vkCmdCopyBufferToImage(copyCmd, stagingBuffer, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &bufferCopyRegion);
    Tool::SetImageLayout(copyCmd, emptyTexture.image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL,
                         subresourceRange);
    ...
    Tool::CheckResult(vkCreateSampler(device->logicalDevice, &samplerCreateInfo, nullptr, &emptyTexture.sampler));
    ...
    Tool::CheckResult(vkCreateImageView(device->logicalDevice, &viewCreateInfo, nullptr, &emptyTexture.view));
        ...
        for (size_t j = 0; j < mesh.primitives.size(); j++)
        {
            ...
            // Vertices
            {
                ...
                if (primitive.attributes.find("COLOR_0") != primitive.attributes.end())
                {
                    ...
                    bufferColors =
                        reinterpret_cast<const float *>(&(model.buffers[colorView.buffer].data[colorAccessor.byteOffset + colorView.byteOffset]));
                }
            ...
            Primitive *newPrimitive =
                new Primitive(indexStart, indexCount, primitive.material > -1 ? materials[primitive.material] : materials.back());
...
void vkglTF::Model::loadImages(tinygltf::Model &gltfModel, OldVulkanDevice *device, VkQueue transferQueue)
{
...
void vkglTF::Model::loadFromFile(std::string filename, OldVulkanDevice *device, VkQueue transferQueue, uint32_t fileLoadingFlags, float scale)
{
    ...
    // Vertex buffer
    Tool::CheckResult(device->CreateBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                                             memoryPropertyFlags,
                                         VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, vertexBufferSize, &vertices.buffer, &vertices.memory));
    // Index buffer
    Tool::CheckResult(device->CreateBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT|VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                                             memoryPropertyFlags,
    ...
    // Manually record the actual min and max from vertexBuffer for verification
    {
        ...
        {
            actualMin.x = std::min(actualMin.x, vertex.pos.x);
            actualMin.y = std::min(actualMin.y, vertex.pos.y);
            actualMin.z = std::min(actualMin.z, vertex.pos.z);

            actualMax.x = std::max(actualMax.x, vertex.pos.x);
            actualMax.y = std::max(actualMax.y, vertex.pos.y);
            actualMax.z = std::max(actualMax.z, vertex.pos.z);
        }
    ...
    Tool::CheckResult(vkCreateDescriptorPool(device->logicalDevice, &descriptorPoolCI, nullptr, &descriptorPool));
    ...
    // Descriptors for per-node uniform buffers
    {
        ...
        if (descriptorSetLayoutUbo == VK_NULL_HANDLE)
        {
            ...
            Tool::CheckResult(vkCreateDescriptorSetLayout(device->logicalDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutUbo));
        }
        for (auto node : nodes)
        {
    ...
    // Descriptors for per-material images
    {
        ...
        if (descriptorSetLayoutImage == VK_NULL_HANDLE)
        {
            ...
            if (descriptorBindingFlags & DescriptorBindingFlags::ImageBaseColor)
            {
                setLayoutBindings.push_back(Init::descriptorSetLayoutBinding(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, VK_SHADER_STAGE_FRAGMENT_BIT,
            ...
            descriptorLayoutCI.bindingCount = static_cast<uint32_t>(setLayoutBindings.size());
            descriptorLayoutCI.pBindings = setLayoutBindings.data();
            Tool::CheckResult(vkCreateDescriptorSetLayout(device->logicalDevice, &descriptorLayoutCI, nullptr, &descriptorSetLayoutImage));
        }
        for (auto &material : materials)
        {
            if (material.baseColorTexture != nullptr)
...
void vkglTF::Model::prepareNodeDescriptor(vkglTF::Node *node, VkDescriptorSetLayout descriptorSetLayout)
{
    if (node->mesh)
    {
        ...
        descriptorSetAllocInfo.pSetLayouts = &descriptorSetLayout;
        descriptorSetAllocInfo.descriptorSetCount = 1;
        Tool::CheckResult(vkAllocateDescriptorSets(device->logicalDevice, &descriptorSetAllocInfo, &node->mesh->uniformBuffer.descriptorSet));
...
glm::mat4 vkglTF::Model::GetModelToStandardTransform()const
{
    // 计算模型包围盒
    glm::vec3 modelMin = actualDimensionsMin;
    glm::vec3 modelMax = actualDimensionsMax;
